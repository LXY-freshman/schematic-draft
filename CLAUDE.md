...
- `scripts/` — symbol/component generators, golden and smoke checks, the
  performance baseline, the Markdown link checker, `sync-to-windows.sh`, the
  real-Electron-window checks `close-guard-window-check.mjs` and
  `shell-style-window-check.mjs`, and the Windows-only `visio-open-check.ps1`,
