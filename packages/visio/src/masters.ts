...
/**
 * Visio's `MasterType` for an ordinary shape.
 *
 * Every master this package writes is one. Declaring a master a connector (541)
 * would hand its path to Visio's routing engine; nothing here wants that, not
 * even the wire, which is a glued line segment rather than a connector — see
 * `wire.ts`.
 */
const SHAPE_MASTER_TYPE = "2";
...
export function mastersPart(masters: readonly VisioMaster[]): OpcPart {
  const entries = masters.map((master) => {
    ...
    return (
      ...
      ` UniqueID="${master.uniqueId}" BaseID="${master.baseId}" PatternFlags="0" Hidden="0" MasterType="${SHAPE_MASTER_TYPE}">` +
